document.addEventListener("DOMContentLoaded", () => {
    ...
    cards.forEach(card => {
        card.addEventListener("click", () => {
            ...
            // certificate_count 증가 요청
            const certificateTitle = title;  // 카드의 title을 사용

            fetch("/increaseCertificateCount", {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
                body: JSON.stringify({
                    certificateTitle: certificateTitle  // 해당 자격증의 title 전달
                })
            })
                .then(response => response.json())
                .then(data => {
                    console.log('Certificate count updated:', data);
